...
function roadLines(mapWidth, mapHeight) {
  ...
  const offsetX = circleRadius; // Horizontal offset to place the circles side-by-side

  context.beginPath();

  // 1. Top left semicircle (clockwise)
  context.arc(
    centerX - offsetX,  // Center of the left circle
    centerY,
    circleRadius,
    Math.PI,            // Start at the leftmost point of the left circle
    0,                  // End at the rightmost point of the left circle
    false               // Clockwise direction
  );

  // 2. Bottom right semicircle (counterclockwise)
  context.arc(
    centerX + offsetX,  // Center of the right circle
    centerY,
    circleRadius,
    Math.PI,            // Start at the leftmost point of the right circle
    0,                  // End at the rightmost point of the right circle
    true                // Counterclockwise direction
  );

  // 3. Top right semicircle (counterclockwise)
  context.arc(
    centerX + offsetX,  // Center of the right circle
    centerY,
    circleRadius,
    0,                  // Start at the rightmost point of the right circle
    Math.PI,            // End at the leftmost point of the right circle
    true                // Counterclockwise direction
  );

  // 4. Bottom left semicircle (clockwise)
  context.arc(
    centerX - offsetX,  // Center of the left circle
    centerY,
    circleRadius,
    0,                  // Start at the rightmost point of the left circle
    Math.PI,            // End at the leftmost point of the left circle
    false               // Clockwise direction
  );
...
function animation(timestamp) {
  ...
  if (otherVehicles.length < (laps + 1) / 3) addVehicle()
...
function movePlayerCar(timeDelta) {
  ...
  playerAngleMoved += playerSpeed * timeDelta; // Increment the angle as the car moves

  // Total distance traveled along the figure-eight path, normalized to 0 - 4π range
  const totalDistance = playerAngleMoved % (4 * Math.PI);

  // Constants for each segment of the path
  const circleRadius = 140;
  const centerXLeft = -arcCenterX; // Center X for the left circle
  const centerXRight = arcCenterX; // Center X for the right circle
  const centerY = 0;
  
  const segmentLength = Math.PI; // Each arc spans π radians
